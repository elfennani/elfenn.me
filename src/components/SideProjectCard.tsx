...
export const SideProjectCard = ({ project }: Props) => {
  ...
  return (
    <div
      key={project.id}
      className="p-2 rounded-2xl bg-background border border-border shadow-xl aspect-[4/3] sm:odd:translate-x-2 sm:even:-translate-x-2 hover:scale-105 hover:rotate-back hover:z-10 transition-transform duration-200 ease-in-out cursor-pointer"
      style={{ transform: `rotate(${randomRotation}deg)` }}
    >
      <div
        className={cn(
          "bg-muted/10 size-full rounded-lg relative flex justify-end flex-col p-4 overflow-hidden from-black/25 via-transparent bg-linear-to-t text-white",
